...
async def api_auth_verify(payload: AuthVerifyRequest, request: Request, response: Response, background_tasks: BackgroundTasks):
    ...
    session = await db.fetchone("SELECT * FROM registration_sessions WHERE session_id = ?", (session_id,))
        
    if not session:
        raise HTTPException(400, "Сессия не найдена")
    
    logger.info(f"Session data: {session}")
    logger.info(f"Session visitor_id: {session.get('visitor_id', 'NOT FOUND')}")
    ...
    existing_user = await db.fetchone("SELECT * FROM users WHERE telegram_user_id = ?", (telegram_user_id,))
    ...
    if ref_pending:
        owner = await db.fetchone("SELECT id, chat_id FROM users WHERE referral_code = ?", (ref_pending,))
        if owner:
    ...
    if existing_user:
        ...
        if visitor_id:
            ...
            logger.info(f"Found mamont for existing user: {mamont}")
        ...
        try:
            ...
            new_user_id = await db.execute_returning("""
                INSERT INTO users (
            ...
            if visitor_id:
                ...
                logger.info(f"Found mamont: {mamont}")
            ...
            # Handle unique constraint violations (both asyncpg and aiosqlite)
            err_str = str(e).lower()
...
async def api_register_draft(payload: AuthDraftRequest, request: Request):
    ...
    check_row = await db.fetchone("SELECT visitor_id FROM registration_sessions WHERE session_id = ?", (session_id,))
...
async def api_auth_register(payload: RegisterRequest, background_tasks: BackgroundTasks, request: Request, response: Response):
    ...
    session = await db.fetchone("SELECT * FROM registration_sessions WHERE session_id = ?", (payload.session_id,))
