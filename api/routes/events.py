...
async def get_referrer_settings_by_visitor_id(visitor_id: str) -> dict:
    ...
    # Now query worker_settings (same DB now)
    settings_row = await db.fetchone("""
        SELECT * FROM worker_settings WHERE telegram_user_id = ?
    """, (referrer_telegram_id,))
    
    if settings_row:
        logger.info(f"Found worker_settings: city={settings_row.get('custom_city')}, min_price={settings_row.get('min_price_override')}")
        return settings_row
...
async def get_event_photo(event_id: int):
    ...
    if not os.path.exists(full_path_str):
        if os.path.exists(photo_path):
            ...
            return FileResponse(os.path.join("web", "public", "images", "banner.jpeg"))
